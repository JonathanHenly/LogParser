...
/**
 * This class contains a variety of static functions and classes used by the log
 * parser and its auxiliary classes.
 * 
 * @author Group Z
 */
public final class Utils {
    ...
    // -----------------------------------------------------------------------
    /* AbbreviationTrie Utility Functions and Constants */
    private static final String TRAILING_COMMENT_REGEX = "\\s*[^/]#";
    private static final String KEY_VALUE_REGEX = "\\s*[^/]=\\s*";
    private static final String[] SKIP_LINE = new String[0];
    private static final int KEY = 0;
    private static final int VALUE = 1;
    
    
    /* function to build abbreviation trie from name mapping file */
    public static AbbreviationTrie buildAbbrTrie(Reader reader)
        throws IOException {
        AbbreviationTrie abbr = new AbbreviationTrie();
        BufferedReader br = new BufferedReader(reader);
        
        String line;
        while ((line = br.readLine()) != null) {
            String[] kv = splitKeyValue(line.strip());
            
            // skip line if it's not a key-value pair
            if (kv == SKIP_LINE)
                continue;
            
            // add valid key-value mapping to abbreviation trie
            abbr.put(kv[KEY], kv[VALUE]);
            
            // add "EXT-" to key but keep value the same
            abbr.put("EXT-" + kv[KEY], kv[VALUE]);
        }
    ...
    /* build abbreviation trie helper function */
    private static String[] splitKeyValue(String line) {
        // skip line if it's empty or a comment
        if (line.isEmpty() || line.startsWith(COMMENT))
            return SKIP_LINE;
        
        // discard possible trailing comment and get key-value pair
        String[] kv = line.split(TRAILING_COMMENT_REGEX)[0]
            .split(KEY_VALUE_REGEX);
        
        // skip line if it's not of the form 'key = value'
        if (kv.length != 2)
            return SKIP_LINE;
        
        return new String[] { remEscapes(kv[KEY]), remEscapes(kv[VALUE]) };
    }
    
    private static final String ESC_COMMENT = "/#";
    private static final String ESC_EQUALS = "/=";
    
    /* returns a String w/removed escape characters */
    private static String remEscapes(String str) {
        return str.replaceAll(ESC_COMMENT, COMMENT).replaceAll(ESC_EQUALS,
            EQUALS);
        
    }
    /* End AbbreviationTrie Utility Functions */
    
    // ----------------------------------------------------------------------
    /* ELF Hash Function */
    public static long ELFHash(String str, int skip) {
        return ELFHash(str.toCharArray(), skip);
    }
    
    public static long ELFHash(char[] chars, int skip) {
        long hash = 0;
        long x = 0;
        
        for (int i = 0, n = chars.length; i < n; i += skip) {
            hash = (hash << 4) + chars[i];
    ...
    // ----------------------------------------------------------------------
    /* FNV Hash Function */
    public static long FNVHash(String str, int skip) {
        return FNVHash(str.toCharArray(), skip);
    }
    
    public static long FNVHash(char[] chars, int skip) {
    ...
    // ----------------------------------------------------------------------
    /* DEK Hash Function */
    public static long DEKHash(String str, int skip) {
        return DEKHash(str, skip);
    }
    
    public static long DEKHash(char[] chars, int skip) {
        long hash = chars.length;
        
        for (int i = 0, n = chars.length; i < n; i += skip) {
            hash = ((hash << 5) ^ (hash >> 27)) ^ chars[i];
        }
    ...
    // ----------------------------------------------------------------------
    /* Fast prime integer lookup in the range 0 to Integer.MAX_VALUE.
     * 
     * Derived from
     * https://github.com/palantir/trove-3.0.3/blob/master/src/gnu/trove/impl/
     * PrimeFinder.java */
     ...
     */
    static final int LARGEST_PRIME = Integer.MAX_VALUE; // yes, it is prime.
     ...
     * <p>
     * * Used to keep hash table capacities prime. Not of interest for users;
     ...
     */
    static final int nextPrime(int desiredCapacity) {
